...
if (-not (Test-Path "$GEN\kernel.asm")) { Write-Host "COMPILE FAILED"; exit 1 }
...
if (-not $elfSize) { Write-Host "LINK FAILED"; exit 1 }
...
QCmd "screendump $BUILD\term_help.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\term_help.ppm" -frames:v 1 -update 1 "$BUILD\term_help.png" 2>$null
$helpSize = (Get-Item "$BUILD\term_help.png" -EA SilentlyContinue).Length
Write-Host "Help: $helpSize bytes"
