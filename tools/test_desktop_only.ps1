# Just test desktop boot - no key presses
...
# Wait 8 seconds for slow clearScreen to finish
...
# Screenshot desktop (no keys)
QCmd "screendump $BUILD\desktop_check.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\desktop_check.ppm" -frames:v 1 -update 1 "$BUILD\desktop_check.png" 2>$null
Write-Host "Desktop: $((Get-Item "$BUILD\desktop_check.png" -EA SilentlyContinue).Length) bytes"

# Now press 1 and wait 8 seconds for terminal to render (clearScreen is slow!)
QCmd "sendkey 1"
Start-Sleep -Seconds 8

QCmd "screendump $BUILD\term_after8s.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\term_after8s.ppm" -frames:v 1 -update 1 "$BUILD\term_after8s.png" 2>$null
Write-Host "Terminal (8s): $((Get-Item "$BUILD\term_after8s.png" -EA SilentlyContinue).Length) bytes"
