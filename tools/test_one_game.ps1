# Test a single game by key
param([string]$Key = "4", [string]$Name = "game")
...
QCmd "screendump $BUILD\test_game.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\test_game.ppm" -frames:v 1 -update 1 "$BUILD\test_game.png" 2>$null
Write-Host "Screenshot: $((Get-Item "$BUILD\test_game.png" -EA SilentlyContinue).Length) bytes"
