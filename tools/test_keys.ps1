# Test different sendkey values to see which ones work
...
# Desktop
QCmd "screendump $BUILD\keys_desk.ppm"
Start-Sleep -Milliseconds 500
ffmpeg -y -i "$BUILD\keys_desk.ppm" -frames:v 1 -update 1 "$BUILD\keys_desk.png" 2>$null
Write-Host "Desktop: $((Get-Item "$BUILD\keys_desk.png" -EA SilentlyContinue).Length) bytes"

# Test sendkey 7 (calculator - known working)
QCmd "sendkey 7"
Start-Sleep -Seconds 5
QCmd "screendump $BUILD\keys_calc.ppm"
Start-Sleep -Milliseconds 500
ffmpeg -y -i "$BUILD\keys_calc.ppm" -frames:v 1 -update 1 "$BUILD\keys_calc.png" 2>$null
Write-Host "Calc(5s): $((Get-Item "$BUILD\keys_calc.png" -EA SilentlyContinue).Length) bytes"

# Press ESC to go back to desktop
QCmd "sendkey esc"
Start-Sleep -Seconds 3

# Test sendkey 1 (terminal)
QCmd "sendkey 1"
Start-Sleep -Seconds 10
QCmd "screendump $BUILD\keys_term.ppm"
Start-Sleep -Milliseconds 500
ffmpeg -y -i "$BUILD\keys_term.ppm" -frames:v 1 -update 1 "$BUILD\keys_term.png" 2>$null
Write-Host "Term(10s): $((Get-Item "$BUILD\keys_term.png" -EA SilentlyContinue).Length) bytes"

# Check serial
$serial = Get-Content "$BUILD\serial.log" -Raw -EA SilentlyContinue
Write-Host "SERIAL ($($serial.Length) chars): $serial"
