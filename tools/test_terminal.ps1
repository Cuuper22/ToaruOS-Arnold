...
QCmd "screendump $BUILD\terminal.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\terminal.ppm" -frames:v 1 -update 1 "$BUILD\terminal.png" 2>$null
Write-Host "Terminal: $((Get-Item "$BUILD\terminal.png" -EA SilentlyContinue).Length) bytes"
