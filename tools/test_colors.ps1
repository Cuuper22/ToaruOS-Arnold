...
# Type "colors" then Enter
foreach ($k in @("c","o","l","o","r","s")) {
    QCmd "sendkey $k"
...
# Screenshot
Remove-Item "$BUILD\screenshot_colors.ppm" -ErrorAction SilentlyContinue
QCmd "screendump $BUILD\screenshot_colors.ppm"
Start-Sleep -Seconds 1

if (Test-Path "$BUILD\screenshot_colors.ppm") {
    Write-Host "Colors: $((Get-Item "$BUILD\screenshot_colors.ppm").Length) bytes"
} else {
    Write-Host "Colors: MISSING"
}
