# Test: type "snake" in terminal to launch Snake
...
# Type "snake" and Enter
foreach ($k in @("s","n","a","k","e")) {
    QCmd "sendkey $k"
...
# Screenshot - should show Snake game
QCmd "screendump $BUILD\term_snake.ppm"
Start-Sleep -Milliseconds 500
ffmpeg -y -i "$BUILD\term_snake.ppm" -frames:v 1 -update 1 "$BUILD\term_snake.png" 2>$null
Write-Host "Snake from terminal: $((Get-Item "$BUILD\term_snake.png" -EA SilentlyContinue).Length) bytes"
