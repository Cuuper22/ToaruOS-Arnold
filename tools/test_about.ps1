...
# Press 8 to launch About dialog
QCmd "sendkey 8"
Start-Sleep -Seconds 3

QCmd "screendump $BUILD\about.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\about.ppm" -frames:v 1 -update 1 "$BUILD\about.png" 2>$null
Write-Host "About: $((Get-Item "$BUILD\about.png" -EA SilentlyContinue).Length) bytes"
