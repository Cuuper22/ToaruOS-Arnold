...
# Press W three times to create 3 overlapping windows
QCmd "sendkey w"
Start-Sleep -Seconds 3
QCmd "sendkey w"
Start-Sleep -Seconds 3
...
QCmd "screendump $BUILD\multi_win.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\multi_win.ppm" -frames:v 1 -update 1 "$BUILD\multi_win.png" 2>$null
Write-Host "Multi-window: $((Get-Item "$BUILD\multi_win.png" -EA SilentlyContinue).Length) bytes"
