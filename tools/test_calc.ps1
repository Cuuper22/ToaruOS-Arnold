...
# Press 7 to launch calculator
QCmd "sendkey 7"
Start-Sleep -Seconds 3

# Type some numbers: 1 + 2 = (enter)
QCmd "sendkey 1"
Start-Sleep -Milliseconds 500
QCmd "sendkey kp_add"
Start-Sleep -Milliseconds 500
...
QCmd "screendump $BUILD\calc.ppm"
Start-Sleep -Milliseconds 500

ffmpeg -y -i "$BUILD\calc.ppm" -frames:v 1 -update 1 "$BUILD\calc.png" 2>$null
Write-Host "Calc: $((Get-Item "$BUILD\calc.png" -EA SilentlyContinue).Length) bytes"
