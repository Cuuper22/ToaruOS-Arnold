# Test terminal: ver and time commands
...
if (-not (Test-Path "$GEN\kernel.asm")) { Write-Host "COMPILE FAILED: $out"; exit 1 }
...
# Screenshot
QCmd "screendump $BUILD\term_ver_time.ppm"
Start-Sleep -Milliseconds 500
ffmpeg -y -i "$BUILD\term_ver_time.ppm" -vf "crop=640:200:0:0,scale=1280:400:flags=neighbor" -frames:v 1 -update 1 "$BUILD\term_ver_time_zoom.png" 2>$null
ffmpeg -y -i "$BUILD\term_ver_time.ppm" -frames:v 1 -update 1 "$BUILD\term_ver_time.png" 2>$null
Write-Host "Screenshot: $((Get-Item "$BUILD\term_ver_time.png" -EA SilentlyContinue).Length) bytes"
